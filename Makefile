...
image: $(OS_IMAGE) $(EFI_OBJS) $(BOOTLOADER_OBJ)
	sudo losetup --offset 1048576 --sizelimit 46934528 /dev/loop99 uefi.img
	sudo mount /dev/loop99 /mnt
	sudo mkdir -p /mnt/EFI/BOOT/
	sudo cp $(EFI_OBJS) /mnt/EFI/BOOT/
	sudo cp $(BOOTLOADER_OBJ) /mnt/EFI/BOOT/BOOTX64.EFI
	sudo umount /mnt
...
%.so: %.o
	$(LD) $(LDFLAGS) $(LDLIB_DIRS) -T$(LD_LINKER_FILE) $(LD_STARTUP_FILE) $< -o $@ $(LDLIBS)
